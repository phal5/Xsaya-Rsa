Shader "Custom/Cel_rev2_Vortex_Tessellation"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        UNITY_INSTANCING_BUFFER_START(UnityPerMaterial)
            ...
            UNITY_DEFINE_INSTANCED_PROP(float, _AxisX)
            UNITY_DEFINE_INSTANCED_PROP(float, _AxisY)
        ...
        Pass
        {
            ...
            Varyings DomainForward(TessellationFactors factors, OutputPatch<TessControlPoint, 3> patch, float3 barycentric : SV_DomainLocation)
            {
                ...
                float axVal = UNITY_ACCESS_INSTANCED_PROP(UnityPerMaterial, _AxisX);
                float ayVal = UNITY_ACCESS_INSTANCED_PROP(UnityPerMaterial, _AxisY);
                float sqVal = UNITY_ACCESS_INSTANCED_PROP(UnityPerMaterial, _UseSquaredLimit);

                float3 deformedPosOS;
                float theta;
                VortexDeform_float(posOS.xyz, tVal, nVal, swVal, axVal, ayVal, sqVal, deformedPosOS, theta);
                ...
                output.normalWS = normalize(RotateVectorZ(unrotNormalWS, theta));
                output.uv = uv;
        ...
        Pass
        {
            ...
            VaryingsOutline DomainOutline(TessellationFactors factors, OutputPatch<TessControlPoint, 3> patch, float3 barycentric : SV_DomainLocation)
            {
                ...
                float3 unrotNormalWS = normalize(TransformObjectToWorldNormal(normOS));
                float3 normalWS = normalize(RotateVectorZ(unrotNormalWS, theta));
        ...
        Pass
        {
            ...
            VaryingsDepth DomainDepth(TessellationFactors factors, OutputPatch<TessControlPoint, 3> patch, float3 barycentric : SV_DomainLocation)
            {
                ...
                float3 deformedPosOS;
                VortexDeform_float(posOS.xyz, tVal, nVal, swVal, axVal, ayVal, sqVal, deformedPosOS);
