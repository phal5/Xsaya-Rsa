...
지정한 **만료일**이 지난 뒤 실행하면, 자신이 들어있는 **데모 폴더 전체를
관리자 권한 없이 스스로 삭제**하는 런처입니다. 만료 전에는 하위 `Game` 폴더의
실제 게임을 대신 실행해 줍니다.
...
- 만료 후 → 안내창 표시 후 `Xsaya-Rsa_Demo/` 폴더 전체 자동 삭제
...
런처는 **자기 자신이 놓인 폴더**만 삭제하며, 그 경로가 아래에 해당하면
삭제를 거부합니다(빈 안내창만 뜨고 아무것도 지우지 않음).
